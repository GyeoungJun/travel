<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            LodgingReservation # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            LodgingReservation
        </v-card-title >        
        ...
        <v-card-actions>
            ...
            <div v-else>
                ...
                <v-btn
                    ...
                    @click="save"
                >
                    RequestCancelALodgingReservation
                </v-btn>
                <v-btn
                    color="primary"
                    text
...
<script>
    ...
    export default {
        name: 'LodgingreservationLodgingReservation',
        components:{
        },
        props: {
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/lodgingReservations'), this.value)
                        } else {
