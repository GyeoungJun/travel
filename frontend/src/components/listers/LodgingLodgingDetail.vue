<template>
    <v-card outlined>
        <v-card-title>
            Lodging # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <Number label="RoomCount" v-model="item.roomCount" :editMode="editMode" @change="change" />
            </div>
        </v-card-text>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/lodgings/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
